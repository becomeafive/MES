<template>
  <div>
    <vol-box :lazy="true"
             ...
             :padding="15">
      <div>
        <iframe :src="model.box1Text"
                frameborder="0"
                style=" width:1000px; height: 80vh"></iframe>
  ...
  methods: {
    getParent() {
      this.$emit('parentCall', ($vue) => {
        this.model.box1Text = JSON.stringify($vue.components.pdfpath)
      })
    },
    getTestData() {
    ...
    open(row) {
      debugger
      this.model.box1Text = 'http://localhost:9991/' + row.Path
      this.modelheight = document.documentElement.clientHeight * 0.9
