...
let extension = {
  ...
  methods: {//事件扩展
    ...
    searchBefore(param) {
      ...
      let ParentId = this.$store.getters.data().treeDemo1.parentId;
      if (ParentId == 0) {
        return false;
      }
      if (treeId == 1) {
      ...
      // param.value = treeId;
      //生成查询条件
      param.wheres.push({ name: 'StationID', value: treeId });
      return true;
    },
    // searchAfter(result) {
    ...
    addAfter(result) {//用户新建后，显示随机生成的密码
      ...
      this.$store.getters.data().treeDemo1.treeId = 1;
      return true;
    },
