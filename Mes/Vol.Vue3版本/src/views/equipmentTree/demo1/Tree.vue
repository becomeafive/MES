<template>
  <div class="com-el-tree">
    <div class="m-title"><i class="el-icon-warning-outline"></i>线体及工位列表</div>
...
export default {
  ...
  methods: {
    load() {
      this.http.post("/api/role/GetLines", {}, true).then((result) => {
    ...
    nodeClick(node, selected) {
      ...
      this.$store.getters.data().treeDemo1.parentId = node.parentId;
